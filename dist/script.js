...
function () {
  ...
  _createClass(Difference, [{
    key: "bimdTriggers",
    value: function bimdTriggers(container, items, count) {
      container.querySelector('.plus').addEventListener('click', function () {
        if (count !== items.length - 2) {
          ...
          var but = document.querySelectorAll('.linkBack');
          but.forEach(function (item) {
            item.addEventListener('click', function () {
              var newItems = Array.from(items).slice(0, count);
...
function () {
  ...
  _createClass(Download, [{
    ...
    value: function downloadItem(path) {
      ...
      element.setAttribute('download', 'nice_pic');
  ...
  _createClass(Form, [{
    ...
    value: function initMask() {
      ...
      function createMask(event) {
        var matrix = '+1 (___) ___-__',
            i = 0,
            def = matrix.replace(/\D/g, ''),
            // Получу все не цифры и заменю их на пробелы
        val = this.value.replace(/\D/g, '');

        if (def.length >= val.length) {
          // Если пользак будет удалять что то, и длина не сойдется, тогда подставим по дефолту значение
          val = def;
        }
          ...
          switch (_context.prev = _context.next) {
            case 0:
              ...
              return regeneratorRuntime.awrap(fetch(url, {
                // если использую запрос, то он асинхрон и об этом надо писать 
                method: "POST",
                body: data
              }));
    ...
    value: function init() {
      ...
      this.forms.forEach(function (item) {
        item.addEventListener('submit', function (e) {
          ...
          _this.postData(_this.path.question, formData).then(function (res) {
            console.log(res);
  ...
  _createClass(VideoPlayer, [{
      ...
      this.btns.forEach(function (btn, i) {
        ...
        btn.addEventListener('click', function (e) {
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            _this.activeBtn = btn; // То на что кликнул пользователь

            if (document.querySelector('iframe#frame')) {
              // Если уже есть , тогда не создаем новый экземпляр плеера
              _this.overlay.style.display = 'flex';

              if (_this.path !== btn.getAttribute('data-url')) {
                // Если урл видео не равен тому, по котормоу мы кликнули, тогда перезаписываем айди видео и открываем плеер уже с другим айди
              ...
              _this.path = btn.getAttribute('data-url'); // УРЛ видео с ютуба
    ...
    value: function createPlayer(url) {
      this.player = new YT.Player('frame', {
        // id куда будет помещаться плеер
        height: '100%',
        width: '100%',
        videoId: "".concat(url),
        // url видео возьмем уже в кнопке 
        events: {
    ...
    value: function onPlayerStateChange(state) {
      try {
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; // Первый родитель у блока с этим селектором

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); // Скопируем ноду
...
function (_Slider) {
  ...
  function MainSlider(reer) {
    var _this;

    _classCallCheck(this, MainSlider);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(MainSlider).call(this, reer)); // Метод чтобы получить доступ к свойствам класса
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      if (n < 1) {
        // Если n меньше единицы, тогда в конец идем
    ...
    value: function showBlock() {
      var selectorBlock = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (item) {
        item.addEventListener('click', function () {
          // При нажатии на кнопку вызываем функцию переключения вперед
          _this2.plusSlides(1); // И передаем туда 1 на сколько слайдер будет переключаться
...
function (_Slider) {
  ...
  _createClass(MiniSlider, [{
    key: "decorizeSlides",
    value: function decorizeSlides(arr) {
      var _this2 = this;

      arr = Array.from(this.slides).filter(function (slide) {
        return slide.tagName !== 'BUTTON';
      });
      arr.forEach(function (slide) {
        slide.classList.remove(_this2.activeClass); // изначально удалим ото всюду этот класс
      ...
      if (this.animate) {
        // Если animate передали, тогда показываем при переключении первому слайду доп фичи
    ...
    value: function nextSlide() {
      if (this.container.classList.contains('feed__slider')) {
        ...
        this.newSlides = Array.from(this.slides).filter(function (slide) {
          return slide.tagName !== 'BUTTON';
        });
      ...
      this.prev.addEventListener('click', function () {
        for (var i = _this3.slides.length - 1; i > 0; i--) {
          if (_this3.slides[i].tagName !== 'BUTTON') {
            var active = _this3.slides[i]; // Последнмй слайд

            _this3.container.insertBefore(active, _this3.slides[0]); // 1 аргумент элемент который хотим поместем, 2 аргумент перед каким помещаем
...
var Slider = function Slider() {
  ...
  // Сюда помещаем те свойства слайдера, которые перед работой ему нужны // Передадим null чтобы не было ошибки с квериселектор
  this.container = document.querySelector(container); // Страница на которой будет работать слайдер

  try {
    this.slides = this.container.children;
  } catch (e) {} // Слайды, которые будут внутри слайдера
