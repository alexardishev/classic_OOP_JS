  ...
  _createClass(VideoPlayer, [{
      ...
      this.btns.forEach(function (btn) {
        btn.addEventListener('click', function () {
          if (document.querySelector('iframe#frame')) {
            ...
            var path = btn.getAttribute('data-url'); // УРЛ видео с ютуба
    ...
    value: function createPlayer(url) {
      this.player = new YT.Player('frame', {
        ...
        height: '100%',
        width: '100%',
        videoId: "".concat(url) // url видео возьмем уже в кнопке 
...
function (_Slider) {
  ...
  function MainSlider(btns) {
    ...
    return _possibleConstructorReturn(this, _getPrototypeOf(MainSlider).call(this, btns)); // Метод чтобы получить доступ к свойствам класса
  }
    ...
    value: function render() {
      ...
      this.btns.forEach(function (item) {
        item.addEventListener('click', function () {
          ...
          _this.plusSlides(1); // И передаем туда 1 на сколько слайдер будет переключаться
...
function (_Slider) {
  ...
  _createClass(MiniSlider, [{
    ...
    value: function decorizeSlides() {
      ...
      this.slides.forEach(function (slide) {
        slide.classList.remove(_this.activeClass); // изначально удалим ото всюду этот класс
      ...
      if (!this.slides[0].closest('button')) {
        this.slides[0].classList.add(this.activeClass); // Если не будет передан, то будет передаваться пустая  строка и ошибки не будет
      }
    ...
    value: function nextSlide() {
      if (this.slides[1].tagName == 'BUTTON' && this.slides[2].tagName == 'BUTTON') {
        this.container.appendChild(this.slides[0]); // Перемещаю слайд

        this.container.appendChild(this.slides[0]); // Перемещаю кнопку 1

        this.container.appendChild(this.slides[0]); // Перемещаю кнопку 2
      ...
      this.prev.addEventListener('click', function () {
        for (var i = _this2.slides.length - 1; i > 0; i--) {
          if (_this2.slides[i].tagName !== 'BUTTON') {
            var active = _this2.slides[i]; // Последнмй слайд

            _this2.container.insertBefore(active, _this2.slides[0]); // 1 аргумент элемент который хотим поместем, 2 аргумент перед каким помещаем
